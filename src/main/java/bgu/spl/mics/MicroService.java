...
public abstract class MicroService implements Runnable {
    private MessageBus messeagebus;
    private String name;
    private HashMap<Class, Callback> messages_callback;
    ...
    public MicroService(String name) {
    	this.name = name;
    	messeagebus = MessageBusImpl.getInstance();
    	messages_callback = new HashMap<>();
    	terminated = false;
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
    	messages_callback.put(type,callback);
    	messeagebus.subscribeEvent(type,this);
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        messages_callback.put(type,callback);
        messeagebus.subscribeBroadcast(type,this);
    }
    ...
    protected final <T> Future<T> sendEvent(Event<T> e) {
        return messeagebus.sendEvent(e);
    }
    ...
    protected abstract void updateDiary();
    /**
    ...
    public final void run() {
        messeagebus.register(this);
        initialize();
        while(!terminated){
            try {
                Message message = messeagebus.awaitMessage(this);
                Callback c = messages_callback.get(message.getClass());
                c.call(message);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            }
        }
        messeagebus.unregister(this);
    }
}
